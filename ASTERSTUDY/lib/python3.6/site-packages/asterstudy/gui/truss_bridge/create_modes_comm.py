def create_modes_comm(material1,material2,element,work_dir,fre):
    import os
    import platform
    sysstr = platform.system()
    if(sysstr =="Windows"):
        print("Call Windoes tasks")
        file = work_dir + '\gui\modes.comm'
    elif(sysstr == "Linux"):
        print ("Call Linux tasks")
        file = work_dir + '/modes.comm'
    else:
...
if __name__ == '__main__':
    ...
    if(sysstr =="Windows"):
        curr_dir =  os.getcwd()
        print('curr_dir:',curr_dir)
        create_modes_comm(material1,material2,element,curr_dir,fre)
    elif(sysstr == "Linux"):
        curr_dir = os.popen('echo `pwd`').read()[0:-1]
        print('curr_dir:',curr_dir)
        create_modes_comm(material1,material2,element,curr_dir,fre)
